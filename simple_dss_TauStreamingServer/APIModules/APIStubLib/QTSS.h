...
FileName:	 QTSS.h
Description: All definitions of  data structure types,including role, objects, attributes, 
             parameters and callback routines to be used frequently soon,in which the code 
			 details of callback routines had been defined extensively in QTSS_Private.cpp.
...
#include "OSHeaders.h"  
#include "../../ServerCore/RTSP/QTSSRTSPProtocol.h"
...
#define QTSS_MAX_SESSION_ID_LENGTH      32 /* 参见RTSPSession::GenerateNewSessionID()  */
#define QTSS_MAX_ATTRIBUTE_NAME_SIZE    64/* see QTSSDictionary.h */
...
typedef SInt32 QTSS_Error; //QTSS_Error definition

// QTSS_AddStreamFlags used in the QTSS_AddStream Callback function
enum
{
    qtssASFlagsNoFlags              = 0x00000000,//used in QTSSFileModule.cpp:DoSetup()
    qtssASFlagsAllowDestination     = 0x00000001,//指明RTP stream的目的地IP
    qtssASFlagsForceInterleave      = 0x00000002,//强迫Interleave
    qtssASFlagsDontUseSlowStart     = 0x00000004,//不用慢启动
    qtssASFlagsForceUDPTransport    = 0x00000008 //只能UDP传输
};
typedef UInt32 QTSS_AddStreamFlags; //QTSS_AddStreamFlags definition
...
typedef UInt32 QTSS_PlayFlags; //QTSS_PlayFlags definition

// Flags for QTSS_Write when writing to a QTSS_ClientSessionObject.
enum 
{
    qtssWriteFlagsNoFlags           = 0x00000000,
    qtssWriteFlagsIsRTP             = 0x00000001,/* 写RTP数据包, used by QTSSFileModule::SendPackets()  */
    qtssWriteFlagsIsRTCP            = 0x00000002,/* 写RTCP数据包 */  
    qtssWriteFlagsWriteBurstBegin   = 0x00000004,/* 开始大量写RTP数据包,used by QTSSFileModule::SendPackets(),RTPStream::Write()  */
    qtssWriteFlagsBufferData        = 0x00000008 /* 发送RTSP Response,used in RTSPSessionInterface::Write() */
};
...
enum
{
    qtssPlayRespWriteTrackInfo      = 0x00000001,/* used in QTSSFileModule.cpp:DoPlay() */
    qtssSetupRespDontWriteSSRC      = 0x00000002 /* used in QTSSCallbacks::QTSS_SendStandardRTSPResponse() */
};
...
enum 
{
    ...
    qtssActionFlagsRead         = 0x00000001,/* used in RTSPSession::run()  */
...
// Events,参见QTSSFile::RequestEvent()
enum
{
...
enum
{
    ...
    qtssRTPTransportModeRecord      = 1 /* used in RTSPRequest::ParseModeSubHeader() */
};
...
/**********************************/
// QTSS API OBJECT TYPES, also considered as CLASS
enum
{
    ...
    qtssModulePrefsObjectType       = FOUR_CHARS_TO_INT('m', 'o', 'd', 'p'), //modp //note the sub type of qtssModuleObjectType
...
typedef UInt32 QTSS_ObjectType; //QTSS_ObjectType定义,15个分量

/**********************************/
// ERROR LOG VERBOSITIES(错误日志级别)
//
...
/* 注意都以qtssAttrDataType开头 */
enum
{
    qtssAttrDataTypeUnknown         = 0,
    qtssAttrDataTypeCharArray       = 1,/* 注意这个和qtssAttrDataTypeQTSS_Object在设置属性和删去属性时尤其要注意,参见QTSSDictionary::SetNumValues() */
    ...
    qtssAttrDataTypeQTSS_Object     = 9,/* QTSSDictionary::CreateObjectValue() */
...
/* 进一步推广见QTSSDictionary.h中的QTSSDictionaryMap类:qtssPrivateAttrModeRemoved = 0x80000000 */
enum
{
    qtssAttrModeRead                = 1,
    qtssAttrModeWrite               = 2,
...
/**********************************/
//BUILT IN SERVER ATTRIBUTES OF OBJECTS(服务器内建属性)
...
/* Id Array of RTPStream Attributes, see also RTPStream.cpp */
enum
{
    ...
    qtssRTPStrSSRC                  = 1,    //read      //UInt32            //SSRC (Synchronization Source) generated by the server. Guaranteed to be unique amongst all streams in the session.
...
//used in RTSPSession::SetupClientSessionAttrs()/RTPSessionInterface::Initialize()
enum
{
    //QTSS_ClientSessionObject parameters. All of these are preemptive safe
    
    qtssCliSesStreamObjects         = 0,    //read      //QTSS_RTPStreamObject//(多重属性)Iterated attribute. All the QTSS_RTPStreamRefs(RTPStream) belonging to this (RTPSession) session.
...
enum 
{
    ...
    qtssRTSPReqIfModSinceDate       = 22,   //read      //QTSS_TimeVal  // If the RTSP request contains an If-Modified-Since header, this is the if-modified date, converted to a QTSS_TimeVal using DateTranslator::ParseDate()


    qtssRTSPReqQueryString          = 23,   //read      //char array  // query string (CGI parameters) passed to the server in the request URL, does not include the '?' separator
...
enum
{
    ...
    qtssSvrIsOutOfDescriptors       = 9,    //read  //Bool16            //true if the server has run out of file descriptors(用光文件描述符), false otherwise
...
enum
{
    ...
    qtssPrefsPlayersReqRTPHeader            = 70,   // "players_requires_rtp_header_info" //Char array //name of player to match against the player's user agent header
    qtssPrefsPlayersReqBandAdjust           = 71,   // "players_requires_bandwidth_adjustment //Char array //name of player to match against the player's user agent header
    qtssPrefsNumParams                      = 72
};
...
enum
{
    ...
    qtssMsgNoMessage                = 0,    //"NoMessage",default!
    ...
    qtssMsgNoColonAfterHeader       = 7,/* 在client's full RTSPRequest的第一行没有冒号,参见RTSPRequest::ParseHeaders() */
    ...
    qtssServerPrefMissing           = 39, /* 未设置服务器预设值 */
...
enum
{
    //QTSS_ConnectedUserObject parameters,参见QTSServerInterface::Initialize()
    ...
    qtssConnectionTimeStorage           = 10,   //read          //QTSS_TimeVal  //Internal, use qtssConnectionTimeConnectedInMsec above,连接时长(ms),参见QTSServerInterface::TimeConnected()
...
//***********************************************/
// TYPE DEFS 
// very important and used frequently! All of complex definitions and type soon after originate from here basically.

// basic type definitions

typedef void*           QTSS_StreamRef;
typedef void*           QTSS_Object; // important and used frequently!
typedef void*           QTSS_ServiceFunctionArgsPtr; /* used in QTSServer::RereadPrefsService()  */
typedef SInt32          QTSS_AttributeID; /* used in QTSSFileModule,就是每个属性的编号 */
typedef SInt32          QTSS_ServiceID;
typedef SInt64          QTSS_TimeVal;

//All QTSS OBJECTS DEFINITIONS
...
//note the following two expression are the same!

typedef QTSS_RTSPStatusCode QTSS_SessionStatusCode;

//***********************************************/
// ROLE PARAMETER BLOCKS
// 每个Role都有一套唯一的参数传递给module
// Each role has a unique set of parameters that get passed to the module.
...
/* used in QTSServer::DoInitRole() */
typedef struct
{
    QTSS_ServerObject           inServer;           // Global server dictionaries
    QTSS_PrefsObject            inPrefs;            // 服务器预设值对象
    QTSS_TextMessagesObject     inMessages;         // 文本消息对象,记录错误日志要用
    QTSS_ErrorLogStream         inErrorLogStream;   // 错误日志流,Writing to this stream causes modules to be invoked in the QTSS_ErrorLog_Role                                                  
    QTSS_ModuleObject           inModule;           // 引用的Module对象
} QTSS_Initialize_Params;

typedef struct
{
    QTSS_ErrorVerbosity         inVerbosity; //日志级别
    char*                       inBuffer;  //要写入日志的缓存的数据
...
/*****************************************************************/
/* need modifying for drm use */
typedef struct
{
    QTSS_ClientSessionObject        inClientSession;/* 必须是RTPSession */
    QTSS_TimeVal                    inCurrentTime; /* usually use value OS::Milliseconds(),记录当前送包时刻 */
    QTSS_TimeVal                    outNextPacketTime;/* 还隔多长时间(相对时间)送下一个包,由当前RTP包的发送情况决定 */
} QTSS_RTPSendPackets_Params;
/*****************************************************************/

/* needed in RTPSession::run() */
typedef struct
{
    QTSS_ClientSessionObject        inClientSession;
...
typedef union
{
    ...
    QTSS_RTPSendPackets_Params          rtpSendPacketsParams; /* refer to RTPSession::run() */
    QTSS_ClientSessionClosing_Params    clientSessionClosingParams;/* refer to RTPSession::run() */
...
typedef struct
{
    void*                           packetData;/* RTP包数据, needed by QTSSRTPFileModule::SendPackets(), very important! provided by QTHintTrack:;GetPacket() */
    QTSS_TimeVal                    packetTransmitTime;/* the absolute time of expected (not actual) to send packet,provided by QTHintTrack:;GetPacket(),comment to RTPStream::Write() */
    QTSS_TimeVal                    suggestedWakeupTime;/*用于计算发送下一个包的等待时间, used by QTSSFileModule::SendPackets()  */
} QTSS_PacketStruct;
...
// MAIN ENTRYPOINT FOR MODULES(模块主入口点)
//
// Every QTSS API must implement two functions: a main entrypoint(主入口点), and a dispatch(分发)
...
//
//
/* 注意这三个函数指针的定义,used in QTSSModule.cpp */

/* 每个Module必备的主函数例程 */
typedef QTSS_Error (*QTSS_MainEntryPointPtr)(void* inPrivateArgs);
/* 每个Module必备的分发函数例程 */
typedef QTSS_Error (*QTSS_DispatchFuncPtr)(QTSS_Role inRole, QTSS_RoleParamPtr inParamBlock); //QTSS_DispatchFuncPtr def

// STUB LIBRARY MAIN
/* 初始化QTSS stub库函数 */
QTSS_Error _stublibrary_main(void* inPrivateArgs, QTSS_DispatchFuncPtr inDispatchFunc); //line 1142, or定义详见QTSS_Private.cpp
...
/********************************************************************/
// note this is added additionally in the current edition!
//  QTSS_GetNumValues
//
//  Returns:    QTSS_NoErr
//              QTSS_BadArgument: Bad argument
...
/********************************************************************/
//  BUILT-IN SERVICES
//this is added just in this edtion!******
//
...
/*****************************************/
//  CLIENT SESSION CALLBACKS /RTP CALLBACK RoUTINES
//
...
//
//  Returns:    QTSS_NoErr
QTSS_Error  QTSS_CreateStreamFromSocket(int inFileDesc, QTSS_SocketStream* outStream);//参见QTSS_Private.cpp定义
