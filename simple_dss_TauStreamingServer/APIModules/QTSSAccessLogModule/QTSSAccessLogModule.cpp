...
FileName:	 QTSSAccessLogModule.cpp
...
static Bool16   sDefaultLogEnabled          = true;//默认滚动日志
...
static Bool16   sDefaultLogTimeInGMT        = true;//默认时间是GTM时间
                            ...
                            QTSS_RTSPSessionObject inRTSPSession, QTSS_CliSesClosingReason *inCloseReasonPtr)
{
    ...
    //获取GMT格式2010-03-16 23:34:54的时间,存入theDateBuffer中,并返回true
...
*/  
    ...
    qtss_sprintf(tempLogBuffer, "%s ", (theDateBuffer[0] == '\0') ? sVoidField : theDateBuffer);   //date* time* //注意时间的写入在这里!
...
/* 向streamingserver.log中写入日志头, 并返回经过处理的当前UNIX时间 */
time_t QTSSAccessLog::WriteLogHeader(FILE *inFile)
{//将生成日志时间写进log头中,再读出,处理后以Unix时间返回
	...
	//获取local time格式2010-03-16 23:34:54的时间,存入theDateBuffer中,并返回true
    ...
    if (result)
    {
		...
		/* 将sLogHeader+"streaming server 5.5.8 GMT"写入当前的streamingserver.log中 */
...
void    WriteStartupMessage()
{
    ...
    //format a date for the startup time
    char theDateBuffer[QTSSRollingLog::kMaxDateBufferSizeInBytes];
	//获取local格式2010-03-16 23:34:54的时间,存入theDateBuffer中,并返回true
    Bool16 result = QTSSRollingLog::FormatDate(theDateBuffer, false);
