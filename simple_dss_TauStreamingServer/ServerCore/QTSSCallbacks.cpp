...
FileName:	 QTSSCallbacks.cpp
Description: Implements QTSS Callback functions which also defined in QTSS_Private.cpp.
...
/* used in QTSSModuleUtils::ReadEntireFile() */
...
// RTSP ROUTINES
//点评:这些例程包装了服务器端对RTSP Request的Client的回应以及对点播的相关响应

/* 实际使用的是RTSPRequestInterface::SendHeader() */
...
/* 实际使用的是RTSPRequestInterface::AppendHeader() */
...
// RTP ROUTINES
/*  引用RTPSession::AddStream(),循环产生一个能唯一标识该RTPSession的RTPStream数组中新的RTPStream的随机数(作为SSRC),搭建一个新的RTPStream,放入RTPStream的数组 */
...
/* 引用RTPSession::Play(),设置好各种参数后发信号开始触发RTPSession和发送RTCP SR包.播放影片 */
...
/* 引用RTPSession::Teardown(),减少该RTSP会话的持有对象计数,设置RTPSession状态为暂停,置空fRTSPSession,再发信号删去该RTPSession */
