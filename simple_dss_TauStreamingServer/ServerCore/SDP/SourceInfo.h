...
class SourceInfo
{
    public:
        ...
        SourceInfo(const SourceInfo& copy);// Does copy dynamically allocated data 复制构造函数,常引用
        ...
        // Returns whether this source is reflectable(反射的).
        Bool16  IsReflectable(); //定义见 SourceInfo.cpp文件
        ...
        struct StreamInfo
        {
            ...
            ~StreamInfo(); // Deletes the memory allocated for the fPayloadName string                eDefaultBufferDelay强制类型转换，默认3         
            
			/* 形参为常引用 */
            void Copy(const StreamInfo& copy);// Does copy dynamically allocated data //Copy为复制构造函数,SourceInfo.cpp中要用
            ...
            UInt16 fTimeToLive; // Ttl(有效存活期) for this stream, 15
		...
		*/
        ...
        // If this source is to be Relayed(中继), it may have "Output" information. This
        ...
        // description of the source, stripped of(剥离) all network info.
    ...
    protected:
        ...
        //utility function used by IsReflectable(反射器IP地址)
